...
\documentclass[dvipsnames, svgnames,
...
lf=\href{http://github.com/xueruini/thuthesis}{ThuThesis},
rf=\href{mailto:xueruini@gmail.com}{Ruini Xue},
randomdots,
dbright=70,
dmindots=6,dmaxdots=9,
dminsize=5pt,dmaxsize=20pt,
logopos={.04\slidewidth,.19\slideheight},
logocmd={\includegraphics[width=1cm]{thu-fig-logo.pdf}}}
...
\onslide*{2}{\thuthesis —— 清华大学学位论文~\LaTeX{} 模板
  ...
  \item 如今：\thuthesis-4.8.1 (2014/12/09)
  \end{itemize}
\medskip

\centering
\includegraphics[width=3cm]{master-cover.pdf}\hspace{1cm}%
\includegraphics[width=2.6cm]{master-toc.pdf}\\
...
\verb|\textbf{Bold}| \pause $\Rightarrow$ \textbf{Bold}\pause 
  \item \verb|\命令[参数]|\\\pause 
\verb|\ziju{1}|\pause ~$\Rightarrow$ {\ziju{1} 增大字距}\pause 
  \end{itemize}
...
\mysb{安装~\thuthesis}
 ...
 \item[下载] 
   \begin{itemize}
   \item CTAN
   \item \href{http://github.com/xueruini/thuthesis}{Github}
   \end{itemize}
 \item[安装]
   \begin{itemize}
...
\begin{slide}{封面样例~(1)}
\includegraphics[width=4.5cm]{bachelor-cover.pdf}\hfill
\includegraphics[width=4.5cm]{master-cover.pdf}
\end{slide}

\begin{slide}{封面样例~(2)}
\includegraphics[width=4.5cm]{doctor-cover.pdf}\hfill
\includegraphics[width=4.5cm]{doctor-ecover.pdf}
\end{slide}
...
\begin{slide}[method=direct]{字号切换}
 ...
 \cmd{sihao} &\cmd{xiaosi} &\cmd{wuhao}&\cmd{xiaowu}  \\
 \cmd{liuhao}&\cmd{xiaoliu}&\cmd{qihao}& \cmd{bahao}  \\\hline
 \end{tabular}
\bigskip 
...
\medskip
\includegraphics[width=3cm]{thu-text-logo.pdf}\hspace{2cm}%
\includegraphics[width=3cm]{thu-text-logo.png}
\end{slide}
...
% 定义密级
\secretlevel{[!red 机密]} \secretyear{[!red 2020]}
\end{SaveVerbatim}
...
\mypauseverb<2->{\UseVerbatim{mypause}}
 \begin{SaveVerbatim}[commandchars=!\[\]]{mypause}
% 中英文关键词
\ckeywords{\TeX，\LaTeX，模板，排版，论文}
\ekeywords{\TeX，\LaTeX，Template，Typesetting，Thesis}
\end{SaveVerbatim}
...
\mysb{先学习}
  ...
  \item 选择一个好编辑器
    \begin{itemize}
    \item 入门：WinEdt$\ldots$
    \item 进阶：Vim (+ \LaTeX Suite) / Sublime Text
    \item 高级：Emacs (+ AUC\TeX + Ref\TeX + preview-\LaTeX) \includegraphics[scale=0.16]{emacs.png}
    \end{itemize}
  \end{itemize}
\end{slide}
...
\item 改进建议
\item 出力维护
\item 感谢：\textbf{\color{blue} alick, oseen, donated, edyfox, Truel}
\end{itemize}
\end{slide}
